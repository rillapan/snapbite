...
export function CardStack<T extends CardStackItem>({
  ...
  const effectiveCardWidth =
    containerWidth > 0
      ? Math.min(cardWidth, Math.round(containerWidth * 0.82))
      : cardWidth;
  const scaleRatio = effectiveCardWidth / cardWidth;
  const effectiveCardHeight = Math.round(cardHeight * scaleRatio);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ height: Math.max(260, effectiveCardHeight + 80) }}
        tabIndex={0}
        onKeyDown={onKeyDown}
      >
  ...
  return (
    <div className="relative h-full w-full bg-white">
      ...
      <div className="relative z-10 flex h-full flex-col justify-end p-6">
        <div className="truncate text-2xl font-black text-white" style={{ letterSpacing: '-0.5px' }}>
          {item.title}
        </div>
        {item.description ? (
          <div className="mt-2 line-clamp-2 text-[14px] text-white/80 leading-[1.6]">
            {item.description}
          </div>
        ) : null}
      </div>
